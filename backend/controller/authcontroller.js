...
export const updateProfile = async (req, res) => {
  try {
    upload.single("photo")(req, res, async (err) => {
      ...
      // Convert societyname to ObjectId if it's a valid string
      if (
        updateData.societyname &&
        mongoose.Types.ObjectId.isValid(updateData.societyname)
      ) {
        updateData.societyname = new mongoose.Types.ObjectId(
          updateData.societyname
        );
      } else {
        return res.status(400).json({ message: "Invalid societyname" });
      }
