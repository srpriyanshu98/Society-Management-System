...
export const getSecurityProtocolById = async (req, res) => {
	...
	try {
		...
		if (!securityProtocol) {
			return res
				.status(404)
				.json({ message: "Security protocol not found" });
		}
...
export const updateSecurityProtocol = async (req, res) => {
	...
	try {
		const updatedSecurityProtocol =
			await SecurityProtocol.findByIdAndUpdate(
				id,
...
export const deleteSecurityProtocol = async (req, res) => {
	...
	try {
		const deletedSecurityProtocol =
			await SecurityProtocol.findByIdAndDelete(id);
