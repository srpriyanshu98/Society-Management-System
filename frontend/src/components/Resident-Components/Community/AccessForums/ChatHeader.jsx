...
export default function ChatHeader({ selectedChat, messages }) {
  ...
  const handleCopy = () => {
    ...
    const allMessages = messages.map((message) => {
      ...
      } else if (message.type === "pdf" || message.type === "document") {
    ...
    <div className="h-20 flex justify-between p-3 relative">
      ...
      <div className="flex space-x-4">
        <button className="bg-gray-100 p-1 w-14 rounded-full">
          <img src="./src/assets/video.svg" alt=""  className="ml-3"/>
        </button>
        <button className="bg-gray-100 p-1 w-14 rounded-full">
          <img src="./src/assets/call.svg" alt="" className="ml-3"/>
        </button>
        <button
          className="bg-gray-100 p-1 w-14 rounded-full"
          onClick={toggleCard}
        >
          <img src="./src/assets/dots.svg" alt="" className="ml-3"/>
        </button>
      </div>
